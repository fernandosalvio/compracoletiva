...
{
	$sql = "SELECT cha_dt_max > DATE_ADD(NOW(),INTERVAL 4 HOUR) as no_prazo FROM chamadas INNER JOIN pedidos on ped_cha = cha_id WHERE ped_id = " . prep_para_bd($ped_id) ;
	$no_prazo = false;
	if($row = mysqli_fetch_array(executa_sql($sql),MYSQLI_ASSOC))
	{
...
//	$mail->SMTPDebug = 2;
	...
	$mail->AddBCC(MAIL_COPY,MAIL_COPY_NAME); // Cópia Oculta
