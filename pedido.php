...
//			redireciona('meuspedidos.php');
		...
		else if ($action == ACAO_SALVAR) // salvar formulário preenchido
		{
			...
			if ($row = mysqli_fetch_array($res,MYSQLI_ASSOC)) 
			{
				...
				$sql.= "ped_dt_atualizacao  = DATE_ADD(NOW(),INTERVAL 4 HOUR) ";
				$sql.= "WHERE ped_id = $ped_id_bd";
				$res = executa_sql($sql);
			}
		...
		{
			$sql = "SELECT (cha_dt_max<DATE_ADD(NOW(),INTERVAL 4 HOUR)) somente_leitura, usr_nome_curto, ped_usr, usr_associado, ped_usr_associado, usr_nome_completo, usr_contatos, prodt_nome, ";
...
-->
         	 <td colspan="5"><div align="right">Contribuição ao EES no caso de associado (já incluído no valor acima)</div></td> 
             <td><?php echo(formata_moeda($total_associado*TAXA_ASSOCIADO));?></td>
           </tr>
<!--    
	   <tr>
...
-->	       
	   <tr>
          	 <th colspan="5"><div align="right">TOTAL FINAL</div></th> 
             <th>R$ <?php echo(formata_moeda($ped_usr_associado==1 ? $total_associado : $total_nao_associado ));?></th>
           </tr>
