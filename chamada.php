...
<?php
		...
		else if ($action == ACAO_SALVAR) // salvar formulário preenchido
		{
			...
			$sql.= "WHERE prod_ini_validade<=DATE_ADD(NOW(),INTERVAL 4 HOUR) AND prod_fim_validade>=DATE_ADD(NOW(),INTERVAL 4 HOUR) AND prod_prodt = " . prep_para_bd($cha_prodt) . " ";
			$res = executa_sql($sql);			
			if($res)
			{
            ...
            <div class="form-group">
             ...
             <div class="col-sm-9">
				...
				<?php
                    ...
                    $sql.= "WHERE prod_ini_validade<=DATE_ADD(NOW(),INTERVAL 4 HOUR) AND prod_fim_validade>=DATE_ADD(NOW(),INTERVAL 4 HOUR) AND forn_archive = '0' AND prod_prodt = " . prep_para_bd($cha_prodt) . " ";
