...
?>
	...
	<?php
		...
		$filtro_disponiveis.="WHERE cha_dt_min <= DATE_ADD(NOW(),INTERVAL 4 HOUR) AND DATE_ADD(NOW(),INTERVAL 4 HOUR)<= cha_dt_max ORDER BY cha_dt_max_original DESC ";
		
		$filtro_anteriores="WHERE DATE_ADD(NOW(),INTERVAL 4 HOUR) > cha_dt_max  ORDER BY cha_dt_entrega_original DESC LIMIT 10"; 	
